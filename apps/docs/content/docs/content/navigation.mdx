...
An href that names a host opens in a new tab; a route stays in the tab. Both `actions` and `cta` are hidden below the `sm` breakpoint, where the header has room for the logo and the navigation toggle and nothing else — so a call to action that has to survive on a phone belongs in `featured`.
...
`repo` also takes a URL, which points the header mark anywhere:
...
That's for a project whose docs repo is private. `github` drives the per-page edit link, the header mark and the [agent manifest](/docs/configuration/ai)'s repository together, so such a project has to leave `github` unset — and a URL is what lets it still show a mark pointing somewhere public.
